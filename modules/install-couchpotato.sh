...
tr -d '\r' < /bandito-box/apps/Bandito-Box/conf/etc/nginx/conf.d/services/service-couchpotato.conf > /bandito-box/.conf/Nginx/services/service-couchpotato.conf
...
#sed -i 's#ssl_key =#ssl_key = /bandito-box/.conf/ssl/domain.tld.key#g' /bandito-box/.conf/CouchPotato.conf
#sed -i 's#ssl_cert =#ssl_cert = /bandito-box/.conf/ssl/domain.tld.pem#g' /bandito-box/.conf/CouchPotato.conf
sed -i 's#data_dir =#data_dir = /bandito-box/.data/CouchPotato#g' /bandito-box/.conf/CouchPotato.conf
sed -i 's#launch_browser = True#launch_browser = False#g' /bandito-box/.conf/CouchPotato.conf
sed -i 's#show_wizard = 0#show_wizard = 1#g' /bandito-box/.conf/CouchPotato.conf

#----> Start service
service couchpotato start && service nginx restart
